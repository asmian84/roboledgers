...
4. Cleanup: Remove "Reference XXXXXXXXX" from descriptions.
  ...
  parseWithRegex(text, metadata = null, lineMetadata = []) {
        ...
        const _statementId = 'RBCCHQ-' + _statementYear + _statementMonth;
        let _seqNum = 0;
  ...
  isCredit(description) {
    const creditKeywords = /DEPOSIT|CREDIT|REFUND|PAYROLL|TRANSFER\s+FROM|E-TRANSFER.*RECEIVED|INTERAC.*REC|DIRECT\s+DEPOSIT|AUTODEPOSIT|AIRBNB.*PAY|ACCOUNT\s+PAYABLE/i;
