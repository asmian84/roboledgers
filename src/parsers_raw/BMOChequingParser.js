...
- Two separate columns for debits and credits!
            ...
            if (amounts.length < 2) {
                ...
                while (i + lookAhead < lines.length && lookAhead <= 3) {
                    ...
                    if (combinedAmounts && combinedAmounts.length >= 2) {
                        ...
                        // Advance past consumed lookahead lines
                        // Use lookAhead - 1 because the for-loop's i++ will add 1
                        i += lookAhead;
                        break;
                    }
                    lookAhead++;
                ...
                if (foundTx) continue;
                // No multi-line match found - fall through to single-line parse
            }
    ...
    parseLineWithAmounts(fullLine, dateStr, isoDate, amountStrings) {
        ...
        const auditData = this.buildAuditData(fullLine, 'BMOChequingParser', { statementId: this._getStmtId(text), lineNumber: ++this._txSeq });
    ...
    _getStmtId(text) {
        ...
        this._cachedStmtId = 'BMOCHQ-' + year + month;
