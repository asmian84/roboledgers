 ...
 */
class CIBCSavingsParser extends BaseBankParser {
    constructor() {
        const formatRules = `
...
- "DEBIT MEMO" and "E-TRANSFER" transactions are common
    ...
    extractTransaction(text, dateStr, originalLine = '') {
        ...
        const auditData = this.buildAuditData(originalLine || text, 'CIBCSavingsParser', { statementId: this._getStmtId(text), lineNumber: ++this._txSeq });
    ...
    _getStmtId(text) {
        ...
        this._cachedStmtId = 'CIBCSAV-' + year + month;
...
window.CIBCSavingsParser = CIBCSavingsParser;
window.cibcSavingsParser = new CIBCSavingsParser();
