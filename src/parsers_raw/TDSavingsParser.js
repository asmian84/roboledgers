 ...
 */
class TDSavingsParser extends BaseBankParser {
    constructor() {
        const formatRules = `
TD CHEQUING FORMAT:
...
4. Merchant Focus: Keep ONLY the merchant name (e.g., "BIG BUCKET CAR").
    ...
    extractTransaction(text, dateStr) {
        ...
        const auditData = this.buildAuditData(text, 'TDSavingsParser', { statementId: this._getStmtId(text), lineNumber: ++this._txSeq });
    ...
    _getStmtId(text) {
        ...
        this._cachedStmtId = 'TDSAV-' + year + month;
